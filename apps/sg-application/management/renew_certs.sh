...
sudo certbot certonly --manual --preferred-challenges http --manual-auth-hook $CORE_MANAGEMENT_DIR/letsencrypt_authenticator.sh --manual-cleanup-hook $CORE_MANAGEMENT_DIR/letsencrypt_cleanup.sh -d grzegorzewski.pl,be.grzegorzewski.pl,mail.grzegorzewski.pl
